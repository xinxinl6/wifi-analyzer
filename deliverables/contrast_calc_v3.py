#!/usr/bin/env python3
# -*- coding: utf-8 -*-
"""WCAG 2.x 色彩对比度计算 — WiFi Analyzer HarmonyOS App (修复后验证版 v3)"""
...
def calc_contrast(fg_color, bg_color, base_bg=None):
    ...
    l2 = relative_luminance(br_c, bg_c, bbb_c)
    ratio = contrast_ratio(l1, l2)
...
# =============================================
# Define all color pairs to test (v3 — 修复后)
# =============================================
...
light_mode_pairs = [
    ...
    ('禁用文字 on 白色卡片', '#8896AB', 'rgba(255,255,255,0.85)', '辅助文字', '3:1'),       # ← 修复

    ('主文字 on 纯白背景', '#1E293B', '#FFFFFF', '正文', '4.5:1'),
    ('次文字 on 纯白背景', '#475569', '#FFFFFF', '正文', '4.5:1'),
    ('弱文字 on 纯白背景', '#64748B', '#FFFFFF', '正文', '4.5:1'),
    ('禁用文字 on 纯白背景', '#8896AB', '#FFFFFF', '辅助文字', '3:1'),                     # ← 修复

    ('主文字 on 页面底色', '#1E293B', '#F8FAFC', '正文', '4.5:1'),
    ('次文字 on 页面底色', '#475569', '#F8FAFC', '正文', '4.5:1'),
    ('弱文字 on 页面底色', '#64748B', '#F8FAFC', '辅助文字', '3:1'),

    # Ping button: white text on SkyBlue (已加深)
    ('按钮文字 on SkyBlue', '#FFFFFF', '#007ACC', '大字/图标', '3:1'),                    # ← 修复
    ('SkyBlue 文字 on 白卡片', '#007ACC', 'rgba(255,255,255,0.85)', '正文', '4.5:1'),      # ← 修复
    ('SkyBlue 文字 on 纯白', '#007ACC', '#FFFFFF', '正文', '4.5:1'),                        # ← 修复

    # Functional colors on light card (已加深)
    ('成功绿 on 白卡片', '#059669', 'rgba(255,255,255,0.85)', '图标/标签', '3:1'),         # ← 修复
    ('警告橙 on 白卡片', '#D97706', 'rgba(255,255,255,0.85)', '图标/标签', '3:1'),         # ← 修复
    ('危险红 on 白卡片', '#DC2626', 'rgba(255,255,255,0.85)', '图标/标签', '3:1'),          # ← 修复

    # Signal level text — 深底白字方案 (v3 修复)
    ('优秀信号: 白字 on 深绿底', '#FFFFFF', '#065F46', '标签文字', '4.5:1'),               # ← 修复
    ('良好信号: 白字 on 深蓝底', '#FFFFFF', '#0C4A6E', '标签文字', '4.5:1'),                # ← 修复
    ('一般信号: 白字 on 深橙底', '#FFFFFF', '#92400E', '标签文字', '4.5:1'),                 # ← 修复
    ('较差信号: 白字 on 深红底', '#FFFFFF', '#7F1D1D', '标签文字', '4.5:1'),                 # ← 修复

    # Tag backgrounds with Sky Blue text
    ('SkyBlue on 标签Bg(浅色)', '#007ACC', 'rgba(0,122,204,0.10)', '标签文字', '3:1'),     # ← 修复
    ('SkyBlue on 标签Bg(选中态浅)', '#007ACC', 'rgba(0,122,204,0.08)', '标签文字', '3:1'),  # ← 修复
    ('半透明SkyBlue(0.75) on 透明', 'rgba(0,122,204,0.75)', '#FFFFFF', '辅助文字', '3:1'),   # ← 修复: 提升到75%不透明度

    # Divider lines
    ('分割线(浅) on 白色', '#F0F2F5', '#FFFFFF', '装饰元素', '-'),
    ('分割线(强) on 白色', '#E8ECF1', '#FFFFFF', '装饰元素', '-'),

    # Aurora primary on white card (已加深)
    ('Aurora主色 on 白卡片', '#2B6CB0', 'rgba(255,255,255,0.85)', '正文', '4.5:1'),         # ← 修复
]

# ===== DARK MODE PAIRS (base_bg = dark blue) =====
DARK_BASE = '#0F2942'
dark_mode_pairs = [
    # Glass text colors (已加亮) on dark glass card
    ('Glass次文字(85%白) on 暗色卡片', '#FFFFFFD9', 'rgba(26,39,68,0.85)', '正文(Dark)', '11:1'),  # ← 修复
    ('Glass弱文字(75%白) on 暗色卡片', '#FFFFFFBF', 'rgba(26,39,68,0.85)', '辅助文字(Dark)', '3:1'), # ← 修复
    ('Glass禁用(60%白) on 暗色卡片', '#FFFFFF99', 'rgba(26,39,68,0.85)', '禁用(Dark)', '3:1'),     # ← 修复

    # Dark mode opaque text colors (已加亮) on dark card
    ('Dark次文字 on 暗色卡片', '#E2EEF9', 'rgba(26,39,68,0.85)', '正文(Dark)', '11:1'),           # ← 修复
    ('Dark弱文字 on 暗色卡片', '#C5DFF0', 'rgba(26,39,68,0.85)', '辅助文字(Dark)', '3:1'),          # ← 修复

    # On pure dark background (page-level)
    ('Glass次文字 on 深蓝背景顶', '#FFFFFFD9', '#0F2942', '正文(Dark)', '11:1'),                    # ← 修复
    ('Glass弱文字 on 深蓝背景顶', '#FFFFFFBF', '#0F2942', '辅助文字(Dark)', '3:1'),                  # ← 修复
    ('Dark次文字 on 深蓝背景顶', '#E2EEF9', '#0F2942', '正文(Dark)', '11:1'),                       # ← 修复
    ('Dark弱文字 on 深蓝背景顶', '#C5DFF0', '#0F2942', '辅助文字(Dark)', '3:1'),                     # ← 修复

    # Sky Blue on dark surfaces (深色模式应使用 LIGHT 变体)
    ('SkyBlue(Light) on 暗色卡片', '#38BDF8', 'rgba(26,39,68,0.85)', '正文(Dark)', '4.5:1'),              # ← 修复: 用LIGHT变体
    ('SkyBlue(Light) on 深蓝背景', '#38BDF8', '#0F2942', '正文(Dark)', '4.5:1'),                           # ← 修复: 用LIGHT变体

    # Functional colors on dark card (已加深)
    ('成功绿 on 暗色卡片', '#059669', 'rgba(26,39,68,0.85)', '图标/标签', '3:1'),                   # ← 修复
    ('警告橙 on 暗色卡片', '#D97706', 'rgba(26,39,68,0.85)', '图标/标签', '3:1'),                   # ← 修复
    ('危险红 on 暗色卡片', '#DC2626', 'rgba(26,39,68,0.85)', '图标/标签', '3:1'),                    # ← 修复

    # Tab bar colors (非激活已加亮)
    ('TabBar激活文字 on TabBar暗底', '#E2E8F0', 'rgba(30,41,59,0.75)', 'Tab文字', '4.5:1'),
    ('TabBar非激活 on TabBar暗底', '#94A3B8', 'rgba(30,41,59,0.75)', 'Tab文字', '4.5:1'),             # ← 修复

    # Tags with text on dark mode (深色模式应使用 LIGHT 变体)
    ('SkyBlue(Light) on 标签Bg(暗色选中)', '#38BDF8', 'rgba(0,122,204,0.15)', '标签文字', '3:1'),           # ← 修复
    ('SkyBlue(Light) on 半透明灰标签Bg(暗色)', '#38BDF8', 'rgba(148,163,184,0.12)', '标签文字', '3:1'),     # ← 修复
    ('警告橙 on 灰标签Bg(暗色)', '#D97706', 'rgba(148,163,184,0.12)', '标签文字', '3:1'),            # ← 修复

    # Aurora colors on dark (深色模式应使用 LIGHT 变体)
    ('Aurora亮色(Light) on 暗色卡片', '#87CEEB', 'rgba(26,39,68,0.85)', '正文(Dark)', '4.5:1'),              # ← 修复: 用LIGHT变体
    ('Aurora亮色 on 暗色卡片', '#87CEEB', 'rgba(26,39,68,0.85)', '正文(Dark)', '4.5:1'),

    # Divider on dark
    ('分割线(暗) on 暗色卡片', 'rgba(255,255,255,0.08)', 'rgba(26,39,68,0.85)', '装饰元素', '-'),

    # Icon circle bg with emoji/text — 改用白字+深底方案
    ('emoji(白色) on SkyActive圆圈', '#FFFFFF', 'rgba(0,122,204,0.25)', '图标', '3:1'),                  # ← 修复: 白字+加深深底

    # Arrow symbol
    ('箭头符号(50%skyblue) on 白卡片', 'rgba(0,122,204,0.5)', 'rgba(255,255,255,0.85)', '装饰元素', '-'),  # ← 修复
    ('箭头符号(SkyBlue) on 暗色卡片', '#007ACC', 'rgba(26,39,68,0.85)', '装饰元素', '-'),                # ← 修复
]


print("=" * 95)
print("色彩对比度分析报告 v3 — WiFi Analyzer (HarmonyOS) 【修复后验证】")
...
if total_fail > 0:
    ...
    print("\n🎉 全部通过！所有 UI 控件均满足 WCAG / 华为审核对比度要求。")

# Save JSON
out_path = os.path.join(os.path.dirname(__file__), 'color-contrast-results-v3.json')
